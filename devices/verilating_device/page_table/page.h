...
public:
	...
	template<class T>
	int writeWord(uint64_t addr, const T in) {
		T dat = in;
		return writeData(addr, sizeof(T), &dat);
	}

	template<class T>
	int writeWords(uint64_t addr, uint64_t num, const T *in, const void *mask = nullptr) {
		if (mask == nullptr) {
	...
	template<class T>
	int readWords(uint64_t addr, uint64_t num, T *data, const void *mask = nullptr) {

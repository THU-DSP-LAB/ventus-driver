...
public:
	...
	uint64_t allocateMemory(uint64_t root, uint64_t v_addr, uint64_t length) {
		...
		for (uint64_t iter = 0ull; iter < length; iter += SV39::PageSize) {
			if (SV39::VAextract((v_addr + iter), 0) != pt_idx[0]) { //may need a new entry in rootPT
				...
				if ((pmm->readWord<uint64_t>(root + pt_idx[0] * sizeof(uint64_t)) & SV39::V) ==
					0ull) { // entry not exist in rootPT
			...
			if (SV39::VAextract(v_addr + iter, 1) != pt_idx[1]) { // may need a new entry in PT1
				...
				if ((pmm->readWord<uint64_t>(pt1_addr + pt_idx[1] * sizeof(uint64_t)) & SV39::V) ==
					0ull) { // entry not exist in PT1
			...
			if (SV39::VAextract(v_addr + iter, 2) != pt_idx[2]) { // may need a new entry in PT2
				...
				if ((pmm->readWord<uint64_t>(pt2_addr + pt_idx[2] * sizeof(uint64_t)) & SV39::V) ==
					0ull) { // entry not exist in PT2
	...
	template<class T>
	T readWordPhysical(uint64_t p_addr) {
		return pmm->readWord<T>(p_addr);
	}

	template<class T>
	int readWordsPhysical(uint64_t p_addr, uint64_t num, T *data, const void *mask = nullptr) {
	...
	template<class T>
	int writeWordPhysical(uint64_t p_addr, const T in) {
		return pmm->writeWord<T>(p_addr, in);
	}

	template<class T>
	int writeWordsPhysical(uint64_t p_addr, uint64_t num, const T *in, const void *mask = nullptr) {
	...
	uint64_t releaseMemory(uint64_t root, uint64_t v_addr) {
		...
		for (uint64_t iter = 0ull; iter < length; iter += SV39::PageSize) {
			auto pt1_addr = SV39::PTE2PA(pmm->readWord<uint64_t>(root + SV39::VAextract(v_addr, 0) * sizeof(uint64_t)));
			auto pt2_addr = SV39::PTE2PA(
			pmm->readWord<uint64_t>(pt1_addr + SV39::VAextract(v_addr, 1) * sizeof(uint64_t)));
